	...
	await page.route("**/api/analyses*", async (route) => {
		...
		if (q)
			filtered = filtered.filter(
				(a) =>
					a.contract_id.includes(q) ||
					a.contract_description.toLowerCase().includes(q.toLowerCase()),
			);
		if (agency)
			filtered = filtered.filter((a) => a.agency.toLowerCase().includes(agency.toLowerCase()));
